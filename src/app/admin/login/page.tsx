...
export default function AdminLogin() {
  const [email, setEmail] = useState("sam786danish@gmail.com"); // Pre-filled as requested
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("Invalid credentials. You do not have authorization.");
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#0B1727] flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl relative z-10">
        <div className="flex flex-col items-center mb-8">
          <div className="w-16 h-16 bg-white rounded-2xl p-2 mb-4 shadow-lg border border-white/20">
            <img src="/logo.png" alt="KICC Logo" className="w-full h-full object-contain" />
          </div>
          <h1 className="text-2xl font-black text-white tracking-tight">KICC Command Center</h1>
          <p className="text-sm text-slate-400 font-medium mt-1 flex items-center gap-1.5">
            <ShieldCheck className="w-4 h-4 text-brand-primary" /> Authorized Personnel Only
          </p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 bg-brand-primary hover:bg-emerald-500 text-white rounded-xl font-bold transition-all shadow-lg shadow-brand-primary/20 flex items-center justify-center gap-2 mt-4"
          >
            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Authenticate & Login"}
          </button>
        </form>
      </div>
    </div>
